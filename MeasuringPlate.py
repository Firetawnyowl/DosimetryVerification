...
class MeasuringPlate(MeasuringPlatePlacement, VoxelStructure.VoxelStructure):
    def __init__(self, y_coordinate, x_rotate, z_rotate, voxel_size=(1, 1, 1), shape=(1, 200, 320)):
        ...
        self.y_coordinate = y_coordinate
        self.structure, self.vox_list = self.voxel_structure()
    ...
    def voxel_structure(self):
        ...
        for line_number in range(self.number_of_voxels_by_z):
            for voxel_number in range(self.number_of_voxels_by_x):
                ...
                structure[voxel_number, line_number] = np.array(voxel)
                vox_list.append(voxel)
        return structure, vox_list

    def cuda_matrix(self):
        matrix = []
        for voxel in self.vox_list:
            matrix.append(self.voxel_corners_after_rotation(voxel))
...
#                 voxel = VoxelStructure.VoxelStructure.voxel_center_after_rotation(self.measuring_plate,
