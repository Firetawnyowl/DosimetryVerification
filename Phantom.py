            ...
            for row_num in range(self.shape[2]):
                for voxel_num in range(self.shape[0]):
                    ...
                    z_coordinate_of_center = self.voxel_size[2] * (row_num + 0.5) - (self.shape[2] *
                                                                                     self.voxel_size[2] / 2)
                    voxel = [x_coordinate_of_center, y_coordinate_of_center, z_coordinate_of_center,
                             self.dose_data[layer_num, row_num, voxel_num]]
                    structure[layer_index, row_num, voxel_num] = np.array(voxel)
                    vox_list.append(voxel)

        return np.array(structure), vox_list
...
class PhantomPart(VoxelStructure.VoxelStructure):
    def __init__(self, phantom, measuring_plate):
        min_layer_index, max_layer_index = phantom.find_layers(measuring_plate.boundaries)
        self.data, self.vox_list = phantom.desired_layers_structure(measuring_plate.boundaries)
        ...
        super().__init__(self.data, phantom.voxel_size, 0, 0)
    ...
    def cuda_matrix(self):
        matrix = []
        for i, voxel in enumerate(self.vox_list):
            matrix.append(self.voxel_corners(voxel))
